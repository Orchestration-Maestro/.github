...
| `merge-queue` (each repository) | A pull request merges through the merge queue, squashed: the queue builds up to five pull requests at once, each on top of the ones ahead of it, and merges up to five together once their checks pass, so two green pull requests cannot merge into a red branch. A lone pull request never waits for a group. Five matches the organization's concurrent macOS runners, since every group runs the macOS leg. GitHub refuses a `merge_queue` rule in an organization ruleset (HTTP 422), so each repository carries `org/repository-rulesets/merge-queue.json`, and the drift check flags one that lacks it |
